...
class BFSCPlus < Khulnasoft::BFSC::BFSC
  # Returns a 4 character known code for a bank
  # TODO: Move this method to the bfsc.rb script
  # in next release
  class << self
...
configure do
  ...
  SecureHeaders::Configuration.default do |config|
    ...
    config.csp = {
      img_src: %w[https://cdn.khulnasoft.com https://khulnasoft.com https://www.google-analytics.com https://stats.g.doubleclick.net],
      default_src: %w['self' https://khulnasoft.com],
...
def bank_details(branch)
  bank_code = BFSCPlus.get_bank_code(branch)

  [BFSCPlus.bank_name_for(branch), bank_code]
end
...
def load_df()
  ...
  arr_keys = ["BANK", "BFSC", "BRANCH", "CENTRE", "DISTRICT", "STATE", "ADDRESS", "CONTACT", "IMPS", "RTGS", "CITY", "ISO3166", "NEFT", "MICR", "UPI", "SWIFT", "BANKCODE"]
  ...
  keys.each_with_index do |bfsc, index|
    
    if bfsc.match(/^[A-Z]{4}0[A-Z0-9]{6}$/)

        lib_bank_details = bank_details(bfsc)

        detail = redis.hgetall(bfsc)
        ...
        dataframe_args["BFSC"].append(bfsc)
...
get '/search' do
  content_type :json
  data = BFSCPlus.filter_banks(params['state'], params['city'], params['bankcode'], params['branch'], params['limit'], params['offset'])
...
get '/places' do
  ...
  if params['bankcode'] != nil && params['state'].nil? && params['district'].nil?
    data = BFSCPlus.get_states(params['bankcode'])
    return JSON.generate(data)

  elsif params['bankcode'] != nil && params['state'] != nil && params['district'].nil?
    data = BFSCPlus.get_districts(params['state'],params['bankcode'])
    return JSON.generate(data)
  
  elsif params['bankcode'] != nil && params['state'] != nil && params['district'] != nil
    data = BFSCPlus.get_branches(params['bankcode'], params['state'], params['district'])
    return JSON.generate(data)
  else
    status 400
  end
